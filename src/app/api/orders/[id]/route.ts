...
// The structure is still implicitly { params: { id: string } }.

export async function GET(request: NextRequest, { params }: any) { // FIX: Changed type to 'any'
    try {
...
export async function PUT(request: NextRequest, { params }: any) { // FIX: Changed type to 'any'
    try {
        // We assume params is { id: string } based on the route structure
        const { id } = params;
